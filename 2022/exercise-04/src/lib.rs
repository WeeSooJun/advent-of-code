...
pub fn run(config: Config) -> Result<(), Box<dyn Error>> {
    ...
    for line in contents.lines() {
        ...
        || (left_schedule.lower <= right_schedule.lower && right_schedule.lower <= left_schedule.upper) || (right_schedule.lower <= left_schedule.lower && left_schedule.lower <= right_schedule.upper) {
            sum += 1;
        }
    }
    println!("{}", sum);
